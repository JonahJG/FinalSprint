...
// setting up user data, pictures, new pictures, url, and error handling to be stored
const Newpic = () => {
  ...
  // fetching the current user data to set the user, user pictures, new pictures, and error (loading) handling
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (pictureUrl) {
      ...
      const lastId = userPictures.length
        ? userPictures[userPictures.length - 1].id
        : 0;
      ...
      // send the updated user data to both localhost:8000/currentUser and localhost:8000/users
      const updateCurrentUserRequest = fetch(
      ...
      const [updateCurrentUserResponse, updateUserResponse] = await Promise.all(
        [updateCurrentUserRequest, updateUserRequest]
      );
      ...
      <div className="picdec">
        ...
        <form onSubmit={handleSubmit}>
          <section className="newpiccontent">
            <label className="enterpic" htmlFor="pictureUrl">
              Enter picture URL:
            </label>
            <input
              type="text"
